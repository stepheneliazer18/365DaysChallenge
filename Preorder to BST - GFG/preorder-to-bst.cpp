...
class Solution{
private:
    Node* constructBST(int& i, int upperBound, int preorder[], int size){
        if(i == size || preorder[i] > upperBound) return NULL;
        
        Node* root = newNode(preorder[i++]);
        root->left = constructBST(i, root->data, preorder, size);
        root->right = constructBST(i, upperBound, preorder, size);
        return root;
    }
public:
    ...
    Node* post_order(int pre[], int size) {
        int i = 0;
        return constructBST(i, INT_MAX, pre, size);
    }
};
